...
let result;
let jsonResults;
const path = './assets/vinetas.json';
    ...
    const connection = async ()  =>{
        try {
            ...
            console.log("Connected to database");
                ...
                // Query the database
                result = await sql.query('SELECT Descripcion, ArticuloDetalle, MIN(PrecioUnitario) as PrecioUnitario FROM BODEGA.dbo.REGISTRO INNER JOIN BODEGA.dbo.DETALLEREGISTRO ON BODEGA.dbo.REGISTRO.IdRegistro = BODEGA.dbo.DETALLEREGISTRO.IdRegistro WHERE Descripcion IS NOT NULL AND Descripcion != \'\' AND LEN(ArticuloDetalle) >= 6 GROUP BY Descripcion, ArticuloDetalle ORDER BY PrecioUnitario ASC;');
